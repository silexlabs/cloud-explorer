	...
	.controller('CEBrowserCtrl', ['$scope', '$rootScope', '$unifileSrv', '$unifileStub', '$ceUtils', function($scope, $rootScope, $unifileSrv, $unifileStub, $ceUtils)
		{
			...
			function accountChanged(account)
			{
				for (var s in account)
				{
					...
					{
						$scope.displayName = account[s]["display_name"];
					}
				}
			}
			function servicesChanged(services)
			{
				for (var s in services)
				{
					...
					{
						isConnected = true;
						$unifileSrv.getAccount( services[s]["name"] );
					}
				}
			}
		...
		return {
			...
			template: "<div> \
						<div class='homeMsg' ng-if=\"!isConnected()\"> \
							Cliquez sur un service dans la liste sur la gauche pour vous y connecter. \
						</div> \
...
//								<a ng-hide=\"file.is_dir\" ng-href=\"{{download()}}\" download=\"{{file.name}}\" target=\"blank\">download</a> \
	...
	.directive('ceBrowser',  function()
	{
		return {
			...
			replace: true,
			template: "<div> \
						<div class=\"ceTitle\">Browse your cloud drives <button class=\"close-btn\" type=\"button\" ng-click=\"abort()\"></button> <button type=\"button\" ng-click=\"logout()\" ng-if=\"isConnected()\">({{displayName}}) Logout</button></div> \
						<div class=\"row-fluid\"> \
							<div class=\"span5\"> \
