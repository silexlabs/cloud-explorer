 ...
 * TODO return CEBlob
 */
function ce_pick(onSuccessCb, onErrorCb) {
 ...
 * @param 
 */
function ce_read(input, onSuccessCb, onErrorCb) {
	__ceInstance.read(input, onSuccessCb, onErrorCb);
}
/**
 ...
 */
function ce_write(target, data, onSuccessCb, onErrorCb) {
	__ceInstance.write(target, data, onSuccessCb, onErrorCb);
}
...
console.log("before extracting path and filename, parsedUrl= "+parsedUrl);
	...
	.factory('$unifileSrv', ['$unifileStub', '$http', 'server.url.unescaped', function($unifileStub, $http, serverUrl)
	{
			...
			for (var si = 0; si < services.length; si++) // FIXME angular 1.1.3 doesn't accept both filter and associative arrays in ng-repeat. As soon as it does, optimize it to make services an associative array
			{
				...
				if (!services[si]["isLoggedIn"])
				{
					var res = $unifileStub.login({service:srvName}, function (status)
						{ console.log('login sucess = '+res.success);
							if (res.success == true)
							{
								services[si]["isLoggedIn"] = true; //console.log('isLoggedIn set for  '+services[si]['name']);
		...
		function getAccount(srv)
		{
			...
			} console.log("getAccount srv = "+srv);
			if (account[srv] == undefined && srv != undefined)
			{
				...
				.success(function(data, status) {
					account[srv] = data; console.log('account set to '+JSON.stringify(account));
				});
			}
	...
	.controller('CEBrowserCtrl', ['$scope', '$rootScope', '$unifileSrv', '$unifileStub', '$ceUtils', function($scope, $rootScope, $unifileSrv, $unifileStub, $ceUtils)
		{
			...
			if (__ceInstance)
			{
				...
				__ceInstance["read"] = function(input, onSuccessCb, onErrorCb) {
...
console.log("path.srv= "+path.srv+"   path.path= "+path.path+"    path.filename= "+path.filename);
					...
					$scope.$apply( function($scope){ 
						$unifileSrv.get(path.srv, path.path+path.filename, function (data) {
							console.log("onSuccess data= "+data); onSuccessCb(data); 
						}, function(error) {
							console.log("onError error= "+error); onErrorCb(error);
						}); 
					});
				};
				__ceInstance["write"] = function(target, data, onSuccessCb, onErrorCb) {
...
console.log('$unifileSrv.currentNav() = ' + JSON.stringify($unifileSrv.currentNav()) );
						if ( $unifileSrv.currentNav().path != undefined )
						{
	...
	.controller('CERightPaneCtrl', ['$scope', '$unifileSrv', '$ceUtils', function($scope, $unifileSrv, $ceUtils)
		{
			...
			{ //console.log('isConnected = '+($scope.srv != null));
			...
			{
//console.log("[CERightPaneCtrl] currentNavChanged in right pane and equals "+JSON.stringify(currNav));
	...
	.controller('CEFileEntryCtrl', ['$scope', '$element', '$unifileSrv', '$unifileStub', 'server.url.unescaped', '$q', '$window', '$ceUtils', function($scope, $element, $unifileSrv, $unifileStub, serverUrl, $q, $window, $ceUtils)
		{
			...
			function connect(srvName)
			{
				if (!$unifileSrv.isConnected(srvName))
				{
					...
					.then(function(cr) { console.log("for "+srvName+" cr.authorize_url= "+cr.authorize_url);
